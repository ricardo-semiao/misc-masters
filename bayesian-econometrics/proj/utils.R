...
# Custom ggsave
ggsave2 <- function(filename, width, k, ...) {
  ggsave(filename, width = width, height = width*k, units = "cm", ...)
}


# Model ICs
get_ic_bvar <- function(data, S_mode, Phi_mean, Sigma_mean) {
  ll <- 0
  
  for (t in (p + 1):n_t) {
    regime <- S_mode[t]
    X_t <- c(1, do.call(c, lapply(1:p, \(l) data[t - l, ])))
    e <- data[t, ] - X_t %*% Phi_mean[[regime]]
    ll <- ll + dmvnorm(e, sigma = Sigma_mean[[regime]], log = TRUE)
  }
  ...
  c(
    ...
    AIC = -2 * ll + 2 * k,
    BIC = -2 * ll + log(n_t) * k,
    HQ = -2 * ll + 2 * k * log(log(n_t)),
    `log(FPE)` = log(exp(-ll / n_t) * (n_t / (n_t - k)))
  )
}
...
prior_minessota <- function(theta = 0.5, sigma_ii = rep(1, K)) {
  se_consts <- map_dbl(mod_var_summ$varresult, ~ .x$coefficients["const", "Std. Error"])
  
  pmap(expand_grid(lag = 1:p, k = 1:K), function(lag, k) {
    sigma_ij <- diag(cov_x)[-k]
    
    row <- numeric(3)
    row[k] <- (sigma_ii[k] / lag)^2
    row[-k] <- ((theta * sigma_ii[-k] * sigma_ij) / (lag * rev(sigma_ij)))^2
    row
  }) %>%
    do.call(rbind, .) %>%
    rbind(as.numeric(se_consts), .) %>%
    {diag(as.vector(.), (1 + 2*K)*K)}
}
...
smooth_probabilities <- function(X, P, Phi_mean, Sigma_mean) {
  alpha <- matrix(0, n_t, m)
  beta <- matrix(1, n_t, m)
  
  for (t in (p + 1):n_t) {
    for (i in 1:m) {
      X_t <- c(1, list_c(map(1:p, \(l) X[t - l, ])))
      e <- X[t, ] - X_t %*% Phi_mean[[i]]
      lik <- dmvnorm(e, sigma = Sigma_mean[[i]])
      
      if (t == p + 1) {
        alpha[t, i] <- lik
      } else {
        alpha[t, i] <- lik * sum(alpha[t - 1, ] * P[, i])
      }
    }
    alpha[t, ] <- alpha[t, ] / sum(alpha[t, ])
  }
  alpha[1:p, ] <- rdirichlet(p, rep(1, m))
  
  for (t in (n_t - 1):1) {
    for (i in 1:m) {
      e <- X[t + 1, ] - X_t %*% Phi_mean[[i]]
      beta[t, i] <- sum(P[i, ] * dmvnorm(e, sigma = Sigma_mean[[i]]) * beta[t + 1, ])
    }
...
rdirf <- function(Phi, Sigma, shock, i, h, order) {
  Phi_i <- Phi[[i]][, order][-1, ]
  Sigma_i <- Sigma[[i]][order, order]
  ...
  L <- t(chol(Sigma_i))
  e <- numeric(K)
  e[shock] <- sd(data_mat[, shock])
  e <- e %*% L
  
  IRF <- matrix(0, nrow = h + p - 1, ncol = K)
  colnames(IRF) <- colnames(data_mat)
  IRF[1:(p-1), ] <- 0
  IRF[p, ] <- e
  
  for (l in 1:h) {
    IRF[l + 1, ] <- (A %*% list_c(map(0:(p-1), ~ IRF[l + .x, ])))[1:K]
  }
  
  IRF[-c(1:(p-1)), ]
}

sample_rdirf <- function(v, i, h, order, ci = 0.95, n = 100) {
  irf_samples <- map(sample(1:n_sims, n), function(s) {
    rdirf(transpose(results_t$Phi[[s]])$val, results_t$Sigma[[s]], v, i, h, order) %>%
      array(dim = c(h, K, 1))
  }) %>%
    {inject(abind::abind(!!!., along = 3))}
  
  list(
    mean = apply(irf_samples, c(1, 2), mean),
    lb = apply(irf_samples, c(1, 2), quantile, probs = (1 - ci) / 2),
    up = apply(irf_samples, c(1, 2), quantile, probs = 1 - (1 - ci) / 2)
  ) %>%
    map(~ `colnames<-`(.x, colnames(data_mat)))
}
...
# from https://github.com/hannes101/LeeStrazicichUnitRoot/tree/master
